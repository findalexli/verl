...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    elif data_source in ["repcount"]:
        from . import repcount

        res = repcount.compute_score(solution_str, ground_truth)
    elif data_source in ["vstar"]:
        from . import vstar

        res = vstar.compute_score(solution_str, ground_truth)
    elif data_source in ["mm_ifeval", "mmifeval"]:
        from . import mmifeval

        res = mmifeval.compute_score(solution_str, ground_truth)
